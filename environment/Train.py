...
class Train:
    def __init__(self, config, train_id, line, capacity, default_speed, max_speed, departure_time, length):
        ...
        self.passengers_to_load = 0
        self.passengers_removed_wait_time = 0
        self.on_line = False
        self.get_position_x()
    ...
    def move(self):
        ...
        if self.distance_to_next_station > 0:
            ...
            # 列车到站，调用列车到站处理函数
            self.arrive_at_final, self.passengers_to_unload, \
                self.passengers_to_load, self.passengers_removed_wait_time = self.arrive_at_station()
        if not self.moving:
            self.moving = True

    def get_passengers_removed_wait_time(self):
        return self.passengers_removed_wait_time
        
    def get_passengers_to_load(self):
        return self.passengers_to_load
    ...
    def arrive_at_station(self):
        ...
        passengers_removed_wait_time = current_station.remove_passengers(passengers_to_load)
        ...
        if next_station_index < 0 or next_station_index >= len(self.line.stations):
            ...
            return self.arrive_at_final, passengers_to_unload, passengers_to_load, passengers_removed_wait_time
        else:
