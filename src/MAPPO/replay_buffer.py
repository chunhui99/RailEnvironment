...
class SharedReplayBuffer(object):
    ...
    def compute_returns(self, next_value, value_normalizer=None):
        ...
        if self._use_proper_time_limits:
            ...
            if self._use_gae:
                # here we use the next value to calculate the return
                self.value_preds[-1] = next_value
                gae = 0
                # self.rewards.shape = [episode_length, n_rollout_threads, num_agents, 1]
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart or self._use_valuenorm:
                        ...
                        if self.algo == "mat" or self.algo == "mat_dec":
                            ...
                            delta = self.rewards[step] + self.gamma * self.value_preds[step + 1] * \
                                    self.masks[step + 1] - self.value_preds[step]
                            gae = delta + self.gamma * self.gae_lambda * self.masks[step + 1] * gae
                            self.returns[step] = gae + self.value_preds[step] # self.returns.shape 201 32 3 1
            else:
                self.returns[-1] = next_value
                for step in reversed(range(self.rewards.shape[0])):
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        data_chunks = batch_size // data_chunk_length  # [C=r*T*M/L] # 1920
        mini_batch_size = data_chunks // num_mini_batch # 1920

        rand = torch.randperm(data_chunks).numpy() # 生成一个随机排列，如果 data_chunks = 5，torch.randperm(5) 可能生成 [2, 3, 1, 4, 0]。
        sampler = [rand[i * mini_batch_size:(i + 1) * mini_batch_size] for i in range(num_mini_batch)]
        ...
        if len(self.share_obs.shape) > 4:
            ...
            # self.share_obs_shape = [T+1,32 3 27]
            share_obs = _cast(self.share_obs[:-1])
            obs = _cast(self.obs[:-1])

        actions = _cast(self.actions) # 19200 1
        action_log_probs = _cast(self.action_log_probs) # 19200 1
        advantages = _cast(advantages) # 19200 1
        value_preds = _cast(self.value_preds[:-1]) # 19200 1
        returns = _cast(self.returns[:-1]) # 19200 1
        masks = _cast(self.masks[:-1]) # 19200 1
        active_masks = _cast(self.active_masks[:-1]) # 19200 1
        # rnn_states = _cast(self.rnn_states[:-1])
        # rnn_states_critic = _cast(self.rnn_states_critic[:-1])
        rnn_states = self.rnn_states[:-1].transpose(1, 2, 0, 3, 4).reshape(-1, *self.rnn_states.shape[3:]) # 19200 1 64
        rnn_states_critic = self.rnn_states_critic[:-1].transpose(1, 2, 0, 3, 4).reshape(-1, *self.rnn_states_critic.shape[3:]) # 19200 1 64

        if self.available_actions is not None:
            available_actions = _cast(self.available_actions[:-1])

        for indices in sampler:
            # len(indices) = 1920 array([ 606,   83, 1556, ...,  542,  353,  140])
            share_obs_batch = []
            obs_batch = []
            rnn_states_batch = []
            rnn_states_critic_batch = []
