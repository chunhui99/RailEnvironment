...
app.layout = html.Div([
    ...
    # 当前时间步显示部分
    html.Div(id="current-time-display", style={"fontSize": 20, "margin": "10px"}),  # 添加显示当前时间步的 Div
...
@app.callback(
    ...
    Output("current-time-display", "children")],  # 添加当前时间步的更新输出
...
def on_next_or_continue(n_next, n_intervals, n_continue, n_stop, is_disabled):
    ...
    if ctx.triggered[0]["prop_id"] == "stop-button.n_clicks":
        return update_figure(), True, f"Current Time Step: {env.world.current_time}"  # 停止自动更新

    # Continue 按钮
    if ctx.triggered[0]["prop_id"] == "continue-button.n_clicks":
        return update_figure(), False, f"Current Time Step: {env.world.current_time}"  # 启动自动更新
    ...
    obs, reward, done, info = env.step(comp_actions)  # 执行一步

    return update_figure(), is_disabled, f"Current Time Step: {env.world.current_time}"  # 保持定时器的状态（禁用或启用）
